...
const CreateHostel = () => {
  ...
  const [demo_secret, setDemo_secret] = useState(false);
  ...
  useEffect(() => {
    if (hasExpired) {
      ...
      const handleDemoAccountChange = (event) => {
        setDemo_secret(event.target.value === 'true');
      };
...
<RadioGroup aria-label="stripe-account" name="stripe-account" value={demo_secret.toString()} onChange={handleDemoAccountChange}>
        <FormControlLabel value="false" control={<Radio />} label="I want to use my own Stripe account" />
        <FormControlLabel value="true" control={<Radio />} label="Use demo Stripe account" />
      </RadioGroup>
