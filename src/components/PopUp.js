...
export default function Popup({ component, pre, word, icon: Icon,  pop = false, bop = false}) {
  ...
  return (
    <div style={bop ? { display: 'inline-block', position: 'relative' } : { display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      <IconButton onClick={handleToggle} style={{ color: 'green', fontSize: "16px" }} ref={anchorRef}>
        {pre}
        {Icon && <Icon />}
        {word}
      </IconButton>
