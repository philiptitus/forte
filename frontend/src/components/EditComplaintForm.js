...
const EditComplaintForm = ({ id,  roomAvailable, accomStatus }) => {
  ...
  const { loading, error, success, complaint } = complaintUpdate;
  ...
  useEffect(() => {
    if (success) {
      enqueueSnackbar("Complaint Info Updated", { variant: 'success' });
    }
    if (error) {
      enqueueSnackbar(error, { variant: 'error' });
    }
