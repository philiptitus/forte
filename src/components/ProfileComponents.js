...
function SelectTextFields({ isC = false, isM = false, isR = false }) {
  ...
  useEffect(() => {
    ...
    if (successMaintenance) {
      enqueueSnackbar("Your Maintenance Request Was Sent To The Hostel Manager", { variant: 'success' });
    }
    if (errormaintenance) {
    ...
    if (errorreview) {
      ...
      }, [successComplaint,successMaintenance,successReview, errorcomplaint, errormaintenance,  errorreview]);
  ...
  return (
    <div>
      <Row style={{ alignItems: "center", justifyContent: "center", textAlign: "center" }}>
        ...
        {isR &&
          <h6>Make A Review For Your Latest Hostel Accommodation
            <br/>

            <i>Which hostel am i reviewing? <b style={{
              color:"red"
            }}>The latest hostel you did not make a review for</b></i>
          </h6>
        }
        {isM &&
          <h6>Make A Maintenance Request</h6>
        }
...
{showPopUp && 
  <Popup  pop={true} component={<SelectTextFields isM={showMaintenance} isC={showComplain} isR={showReviews}/>}  icon={AddIcon} pre="NEW:  " />
